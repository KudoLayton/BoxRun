...
public class Tutorial_0 : MonoBehaviour {
	...
	void Start () {
		...
		stage = 0;
		text_2.text = "";
		scriptdata = new StreamReader( new FileStream("..\\Script\\Script_tutorial_0.txt", FileMode.Open));
	...
	void Stageselect(){
		if(GameObject.Find ("Player")){
			switch(stage){
			case 1:
				if(new Vector3(0f, 1f, 4f) == new Vector3(Mathf.Round(GameObject.Find ("Player").transform.position.x), Mathf.Round(GameObject.Find ("Player").transform.position.y), Mathf.Round(GameObject.Find ("Player").transform.position.z))){
					stage++;
					GameObject.Find("Player").GetComponent<Player_move>().movecontrol = false;
			...
			case 4:
				scriptdata.Close();
				if(new Vector3(0f, 1f, 8f) == new Vector3(Mathf.Round(GameObject.Find ("Player").transform.position.x), Mathf.Round(GameObject.Find ("Player").transform.position.y), Mathf.Round(GameObject.Find ("Player").transform.position.z))){
					text.text = "";
					text_2.text = "";
				}
				break;
